...
import { parseGeographyDocx, type GeographyParseResult } from '@/lib/geographyDocxParser';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('[geography/import-docx] 开始导入:', file.name);
    ...
    const parsedData = parseGeographyDocx(rawText, file.name);
    if (unitId) parsedData.unitId = unitId;

    const importId = `geography_docx_import_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ...
    if (isSupabaseConfigured && supabase) {
      try {
        ...
        if (insertError) {
          console.error('[geography/import-docx] Supabase insert error:', insertError);
          setServerData(importId, parsedData);
        } else {
          console.log('[geography/import-docx] 已保存到 Supabase, id:', importId);
        }
      } catch (supabaseError) {
        console.error('[geography/import-docx] Supabase error:', supabaseError);
        setServerData(importId, parsedData);
      }
    } else {
      setServerData(importId, parsedData);
      console.log('[geography/import-docx] 已保存到 serverStorage, key:', importId);
    }
    ...
    console.error('[geography/import-docx] 导入失败:', error);
    return NextResponse.json({
      success: false,
...
export async function GET() {
  ...
  try {
    const { data, error } = await supabase
      ...
      .eq('user_id', USER_ID)
      .eq('textbook_id', 'geography-docx')
      .order('imported_at', { ascending: false });

    if (error) {
      console.error('[geography/import-docx] 查询失败:', error);
    ...
    console.error('[geography/import-docx] GET error:', error);
    return NextResponse.json({ success: false, message: '查询失败' }, { status: 500 });
  }
}
