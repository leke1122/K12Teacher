...
import type { Concept } from '@/lib/politicsDocxParser';
...
export default function GuidedLearning({ concepts }: GuidedLearningProps) {
      ...
      <CardContent className="space-y-4">
        ...
        {step === 'question' && (
          <div className="space-y-3">
            <p className="text-sm font-medium text-slate-700">Q：为什么 {concept?.name} 是本课的核心理论？</p>
            <div className="flex flex-wrap gap-2">
            ...
            {showHint && (
              <p className="text-xs text-amber-600">提示：从理论地位、历史作用、现实意义三个角度思考。</p>
            )}
          </div>
        )}
        ...
        {step === 'reflect' && (
          <div className="space-y-3">
            <p className="text-sm text-slate-700">对比思考：如果这个理论不存在，社会形态演进会有什么不同？</p>
